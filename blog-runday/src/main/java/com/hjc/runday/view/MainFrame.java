...
public class MainFrame extends JFrame implements MouseListener {
	...
	public MainFrame() {//无参构造，创建对象。并在main函数中调用
		//2.2
		try (
				InputStream inputStart = new ClassPathResource("Image/hh1.png").getInputStream();
				InputStream inputHelp = new ClassPathResource("Image/hh2.png").getInputStream();
			...
			start = new JLabel(new ImageIcon(ImageIO.read(inputStart)));//ImageIcon:图标
			start.setBounds(350,320,150,40);
			start.setEnabled(false);//false按钮为灰色
			start.addMouseListener(this);
			this.add(start);

			help = new JLabel(new ImageIcon(ImageIO.read(inputHelp)));
			...
			this.setIconImage(new ImageIcon(ImageIO.read(input115)).getImage());//logo
	...
	class MainPanel extends JPanel{//创建的MainPanel类，在MainFrame中调用
		...
		public MainPanel() {
			try (
					InputStream inputStream = new ClassPathResource("Image/main.png")
							.getInputStream();
			) {
