...
public class Person {//1.声明属性
	...
	private void init() {//2.2
		...
		for (int i = 0; i < images.length; i++) {//2.3
			try (
					InputStream inputStream = new ClassPathResource(String.format("Image/%d.png", i + 1))
							.getInputStream();
			) {//2.5
				images[i] = ImageIO.read(inputStream);//2.4 /Users/admin/Documents/workspace/CS_Test/runday/image
