package com.hjc.runday.controller;

import com.hjc.runday.model.*;
import com.hjc.runday.view.EndFrame;
...
public class GamePanel extends JPanel implements KeyListener{
	...
	public GamePanel() {
		...
		//2.2读取图片文件
		try(
				InputStream inputBackground = new ClassPathResource("Image/cc.png").getInputStream();
				InputStream inputScore = new ClassPathResource("Image/a12.png").getInputStream();
				InputStream inputPause = new ClassPathResource("Image/b2.png").getInputStream();
				InputStream inputProceed = new ClassPathResource("Image/b1.png").getInputStream();
				){
			background =ImageIO.read(inputBackground);//跑酷背景
			score =ImageIO.read(inputScore);//得分背景
			pause = ImageIO.read(inputPause);
			proceed = ImageIO.read(inputProceed);
		}catch(IOException e){
			e.printStackTrace();
		}
	}
